...
exports.UniswapV2PairSwapDecoder = exports.UniswapV2RouterSwapDecoder = void 0;
...
const getTokens = (tokenA, tokenB) => {
    return tokenA < tokenB ? [tokenA, tokenB] : [tokenB, tokenA];
};
const computePairAddress = (factory, initcodeHash, tokenA, tokenB) => {
    ...
    return ethers_1.ethers.utils.getCreate2Address(factory, salt, initcodeHash);
};
class UniswapV2RouterSwapDecoder extends types_1.Decoder {
    constructor() {
        super(...arguments);
        this.functions = {
    ...
    async decodeCall(state, node) {
        ...
        const inputAmountIn = inputs['amountIn'] ? inputs['amountIn'].toBigInt() : undefined;
        const inputAmountOutMin = inputs['amountOutMin'] ? inputs['amountOutMin'].toBigInt() : undefined;
        const inputAmountOut = inputs['amountOut'] ? inputs['amountOut'].toBigInt() : undefined;
        const inputAmountInMax = inputs['amountInMax'] ? inputs['amountInMax'].toBigInt() : undefined;
        ...
        if ((0, utils_1.hasReceiptExt)(node)) {
            const swapEventSelector = 'Swap(address indexed sender, uint amount0In, uint amount1In, uint amount0Out, uint amount1Out, address indexed to)';
            const abi = new ethers_1.ethers.utils.Interface([abi_1.EventFragment.from(swapEventSelector)]);
            const swapEvents = node.logs.filter((log) => (0, utils_1.hasTopic)(log, swapEventSelector));
            const [firstToken0, firstToken1] = getTokens(path[0], path[1]);
            const firstPairAddress = computePairAddress(routerInfo.factory, routerInfo.initcodeHash, firstToken0, firstToken1);
            const [lastToken0, lastToken1] = getTokens(path[path.length - 2], path[path.length - 1]);
            const lastPairAddress = computePairAddress(routerInfo.factory, routerInfo.initcodeHash, lastToken0, lastToken1);
            ...
            if (firstSwapEvent) {
                ...
                const eventAmount0In = parsedEvent.args['amount0In'] ? parsedEvent.args['amount0In'].toBigInt() : undefined;
                const eventAmount1In = parsedEvent.args['amount1In'] ? parsedEvent.args['amount1In'].toBigInt() : undefined;
                swapResult.amountIn =
                    firstToken0 === path[0]
                        ? eventAmount0In
                        : eventAmount1In;
            ...
            if (lastSwapEvent) {
                ...
                const eventAmount0Out = parsedEvent.args['amount0Out'] ? parsedEvent.args['amount0Out'].toBigInt() : undefined;
                const eventAmount1Out = parsedEvent.args['amount1Out'] ? parsedEvent.args['amount1Out'].toBigInt() : undefined;
        ...
        if (outputs) {
            if (!swapMetadata.fee) {
                ...
                const lastAmount = amounts[amounts.length - 1] ? amounts[amounts.length - 1].toBigInt() : undefined;
                    ...
                    switch (swapMetadata.output) {
                        case 'tokens':
                            const balanceOfCalls = node.children
                                ...
                                .filter((v) => (0, utils_1.hasSelector)(v.calldata, 'balanceOf(address)'));
                            // pull out the balanceOf calls
                            const initialBalance = ethers_1.BigNumber.from(balanceOfCalls[0].returndata);
                            const finalBalance = ethers_1.BigNumber.from(balanceOfCalls[balanceOfCalls.length - 1].returndata);
                            swapResult.amountOut = finalBalance.sub(initialBalance).toBigInt();
                            break;
                        case 'eth':
                            const calls = node.children.filter((v) => v.type === 'call');
                            const lastAmount = calls[calls.length - 1].value ? calls[calls.length - 1].value.toBigInt() : undefined;
    ...
    consumeSwaps(state, node) {
        if (!(0, utils_1.hasTraceExt)(node))
            return;
        node.children
            .filter((call) => call.type === 'call')
            .filter((call) => (0, utils_1.hasSelector)(call.calldata, 'swap(uint256,uint256,address,bytes)'))
            .forEach((call) => {
            state.consume(call);
            call.children
                .filter((v) => v.type === 'call' && (0, utils_1.hasSelector)(v.calldata, 'transfer(address,uint256)'))
                .forEach((v) => state.consumeTransfer(v));
        });
    }
    consumeTokenInputSwap(state, node) {
        if (!(0, utils_1.hasTraceExt)(node))
            return;
        const calls = node.children.filter((v) => v.type === 'call');
        state.consumeTransferFrom(calls[0]);
    }
    consumeETHInputSwap(state, node) {
        if (!(0, utils_1.hasTraceExt)(node))
            return;
        const calls = node.children.filter((v) => v.type === 'call');
        // weth deposit
    ...
    consumeETHOutputSwap(state, node) {
        if (!(0, utils_1.hasTraceExt)(node))
            return;
        const calls = node.children.filter((v) => v.type === 'call');
        // weth withdraw
...
exports.UniswapV2RouterSwapDecoder = UniswapV2RouterSwapDecoder;
...
class UniswapV2PairSwapDecoder extends types_1.CallDecoder {
    constructor() {
        super();
        this.functions['swap(uint256 amount0Out, uint256 amount1Out, address to, bytes data)'] = this.decodeSwap;
    }
    async getDeploymentForPair(state, address) {
    ...
    async decodeSwap(state, node, inputs, outputs) {
...
exports.UniswapV2PairSwapDecoder = UniswapV2PairSwapDecoder;
