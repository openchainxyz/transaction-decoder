...
class UniswapV2RouterSwapDecoder extends types_1.Decoder {
    ...
    async decodeCall(state, node) {
        ...
        if (swapMetadata.exactIn) {
            swapResult.amountIn = swapMetadata.input === 'eth' ? node.value.toBigInt() : inputs['amountIn'].toBigInt();
            swapResult.amountOutMin = inputs['amountOutMin'].toBigInt();
        }
        else {
            swapResult.amountOut = inputs['amountOut'].toBigInt();
            swapResult.amountInMax = inputs['amountInMax'].toBigInt();
        }
        ...
        if ((0, utils_1.hasReceiptExt)(node)) {
            ...
            if (firstSwapEvent) {
                ...
                swapResult.amountIn =
                    firstToken0 === path[0]
                        ? parsedEvent.args['amount0In'].toBigInt()
                        : parsedEvent.args['amount1In'].toBigInt();
            }
            if (lastSwapEvent) {
                const parsedEvent = abi.parseLog(lastSwapEvent);
                swapResult.amountOut =
                    lastToken0 === path[path.length - 1]
                        ? parsedEvent.args['amount0Out'].toBigInt()
                        : parsedEvent.args['amount1Out'].toBigInt();
            }
        }
